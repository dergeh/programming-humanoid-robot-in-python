...
class AngleInterpolationAgent(PIDAgent):
    def __init__(self, simspark_ip='localhost',
        ...
        self.keyframes = ([], [], [])
        self.t0=-1 # starting time of the agent gotten from simspark
    ...
    def angle_interpolation(self, keyframes, perception):
        target_joints = {}
        # if keyframes are empty return
        if (keyframes==([],[],[])):
            return target_joints
        names, times, keys = keyframes

        # set starting time of the keyframe
        if self.t0==-1:
            self.t0=perception.time
        ...
        for i in range(0,len(names)):
                ...
                # if time limit of joint is reached skip joint for interpolation
                if time_rel> times[i][-1]:
                    skipped_joints+=1
                    # if every joint is skipped reset time and keyframe
                    if skipped_joints==len(names):
                        print(" resetting works")
                        self.t0=-1
                        self.keyframes=([],[],[])
                        return {}
                ...
                # else get previous position
                else:
                    p0=(0,keys[i][time_index-1][0]) # start point
                ...
                # cubic bezier split for every point 
                ...
                #    test.write("%f,%f\n"%(b_t,b[1]))
                target_joints[names[i]]=b[1] #add interpolated value to target_joint dicct
